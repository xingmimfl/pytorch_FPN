...
start_step = 0
end_step = 250000
lr_decay_steps = {50000, 100000, 150000, 200000}
lr_decay = 1./10
...
for step in range(start_step, end_step+1):
    ...
    loss = net.loss + net.rpn_net.loss
    if _DEBUG:
        tp += float(net.tp)
        tf += float(net.tf)
        fg += net.fg_cnt
    ...
    if step % disp_interval == 0:
        ...
        if _DEBUG:
            ...
            log_print('\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box: %.4f' % (
                net.rpn_net.cross_entropy.data.cpu().numpy()[0], net.rpn_net.loss_box.data.cpu().numpy()[0],
